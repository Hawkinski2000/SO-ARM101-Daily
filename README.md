...
- What I did: I (probably) found the source of the choppy joint plots today.
- What I learned: I tested the co-trained `aravindhs-NV/grootn16-finetune_sreetz-so101_teleop_vials_rack_left_sim_and_real` model and it didn't have a noticable effect on the joint plots. I added a print statement to measure the duration of `actions = policy.get_action(obs)`, and just by observing the rate of the print statements themselves (not the duration values), I noticed the frequency of the loop was not only perfectly synchronized with the waves in the joint plots, but it was proportional to the brightness of the workspace. According to Claude, some cameras have `auto_exposure` enabled by default. Apparently this can cause variable camera capture latency depending on the brightness, which is what caused a difference in frequency of the plots and eval loop depending on the brightness, so I modified the code to use a fixed manual exposure value. This was not the main issue causing the choppy joint plots, however. That turned out to be (probably) an issue related to the `action_horizon`, specifically the "action-chunking seam" according to Claude. It suggested using a "temporal ensembling" approach. It seems to have improved the performance, and the joint plots seem much smoother. After some tuning of the parameters, I was able to observe what was probably some of the best arm behavior so far. It was often able to pick up a vial, and it usually got reasonably close to placing it in the rack. At times it would even go back for a second vial, which is something I don't think I saw previously. It's clear that the actual inference/eval code itself seems to have been a major bottleneck, which is a bit ironic. I even tested GapONet and it flailed noticably less, however its behavior was still pretty much nonsensical. I will have to investigate that further.
- What's next: I'll look into this temporal ensembling approach in more detail tomorrow. Claude also suggested just lowering the `action_horizon` value, so maybe that would be worth testing as well instead of using an entirely new approach. I'll also try experimenting with the parameters some more to see if I can improve the performance any further.